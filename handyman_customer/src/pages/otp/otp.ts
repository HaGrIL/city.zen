...
@Component({
  ...
  providers: [Firebase, ClientService]
})
export class OtpPage {
  ...
  constructor(public params: NavParams, public navCtrl: NavController, public platform: Platform,
    public alertCtrl: AlertController, public loadingCtrl: LoadingController, private app: App, private translate: TranslateService,
    public toastCtrl: ToastController, private firebase: Firebase, private clientService: ClientService, private events: Events) {
  ...
  loginUser(token) {
    ...
    this.clientService.login({ token: token, role: "customer" }).subscribe(res => {
      this.dismissLoading();
  ...
  sendOtpPhone(phone) {
    ...
    this.firebase.verifyPhoneNumber(phone, 60).then((credential) => {
      ...
      if (this.platform.is('ios')) {
        this.verfificationId = credential;
      }
